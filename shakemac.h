 ...
 *
 * Authors:  David Leon Gil (coruus@gmail.com)
 *           Yahoo (dgil@yahoo-inc.com)
 * License:  Apache 2
 *
...
static inline int shake256_init(keccak_sponge* const restrict sponge) {
  if (sponge == NULL) {
...
static inline int shake256_absorb(keccak_sponge* const restrict sponge,
...
static inline int shake256_squeeze(keccak_sponge* const restrict sponge,
...
static inline int shake256(uint8_t* const restrict out,
                           const size_t outlen,
                           const uint8_t* const restrict in,
                           const size_t inlen) {
  int err = 0;
...
static inline mac_init(keccak_sponge* const restrict sponge,
...
static inline mac_cached(keccak_sponge* const restrict sponge,
                              const uint64_t* const state) {
  int err = 0;
  err = shake256_init(&sponge);
  if (err != 0) {
    return err;
...
static inline mac_cache_key(uint64_t* const state,
                            const uint8_t* const key,
                            const size_t keylen) {
  int err = 0;
  keccak_sponge sponge;
...
#undef FLAG_ABSORBING
#undef FLAG_SQUEEZING
#undef FLAG_SPONGEPRG
#undef DS_SHAKE
#undef DS_MACKEY
